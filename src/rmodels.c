...
#endif
#if defined(SUPPORT_FILEFORMAT_GLTF)
static Model LoadGLTF_FromMemory(uint8_t* fileData, int dataSize, const char* fileName);
...
Model LoadModelFromMemory(unsigned char* fileData, int dataSize, const char* fileName)
{
    Model model = { 0 };
...
//#endif
//#if defined(SUPPORT_FILEFORMAT_GLTF)
    if (IsFileExtension(fileName, ".gltf") || IsFileExtension(fileName, ".glb")) model = LoadGLTF_FromMemory(fileData, dataSize, fileName);
//#endif
//#if defined(SUPPORT_FILEFORMAT_VOX)
...
extern void CAssetsManager(const char* filepath, unsigned char** outData, int* outSize, char* extension);

// Load image from different glTF provided methods (uri, path, buffer_view)
static Image LoadImageFromCgltfImageFromMemory(cgltf_image* cgltfImage, const char* texPath)
{
    Image image = { 0 };
    ...
    if (cgltfImage->uri != NULL)     // Check if image data is provided as an uri (base64 or path)
    {
        ...
        else     // Check if image is provided as image path
        {
            ...
            int size = 0;
            CAssetsManager(cgltfImage->uri, &data, &size, &extension);
            image = LoadImageFromMemory(extension, data, size);
        }
    }
...
// Load glTF file from memory, .gltf and .glb supported 
static Model LoadGLTF_FromMemory(uint8_t* fileData, int dataSize, const char* fileName)
{
    /*********************************************************************************************
    ...
    if (result == cgltf_result_success)
    {
        ...
        for (unsigned int i = 0, j = 1; i < data->materials_count; i++, j++)
        {
            ...
            if (data->materials[i].has_pbr_metallic_roughness)
            {
                ...
                {
                    Image imAlbedo = LoadImageFromCgltfImageFromMemory(data->materials[i].pbr_metallic_roughness.base_color_texture.texture->image, texPath);
                ...
                {
                    Image imMetallicRoughness = LoadImageFromCgltfImageFromMemory(data->materials[i].pbr_metallic_roughness.metallic_roughness_texture.texture->image, texPath);
                ...
                {
                    Image imNormal = LoadImageFromCgltfImageFromMemory(data->materials[i].normal_texture.texture->image, texPath);
                ...
                {
                    Image imOcclusion = LoadImageFromCgltfImageFromMemory(data->materials[i].occlusion_texture.texture->image, texPath);
                ...
                {
                    Image imEmissive = LoadImageFromCgltfImageFromMemory(data->materials[i].emissive_texture.texture->image, texPath);
